...
| 지도 vs 무지도 정면비교 | **같은 데이터**서 결함 라벨 N개 스윕 → **교차점 ≈ 20개**(미만이면 무지도 우위) 실측 | `scripts/head_to_head.py` |
...
## 지도 vs 무지도 — 같은 데이터에서 정면 비교, "라벨이 몇 개면 지도가 이기나"

위 두 트랙(지도분류·무지도 AD)은 서로 *다른 데이터셋*(NEU vs MVTec)이라 "같은 이미지로 누가 낫나"를
말할 수 없었다. 이 트랙은 그 빈틈을 메운다 — **한 데이터셋(MVTec screw)** 위에서 두 패러다임을
**같은 고정 테스트셋·같은 지표(image AUROC)**로 붙이고, 실무의 진짜 질문에 숫자로 답한다:

> **"결함 라벨을 N개 줄 때, 지도학습이 라벨 0개의 무지도 이상탐지를 *언제부터* 이기나?"**

**변수를 '감독 방식' 하나로 고립한 통제 설계** (`scripts/head_to_head.py`):
- 두 방법 모두 **같은 ImageNet 사전학습 백본**(resnet18) — 백본 용량 차이를 제거.
- 두 방법 모두 **같은 정상(양품) 이미지 전부**를 본다(정상은 싸다). 유일한 차이는 지도가
  **추가로 N개의 라벨된 결함**을 쓴다는 것 — 무지도는 결함을 0개 본다.
- 불균형(정상 321 ≫ 결함 N)은 **클래스 균형 샘플러 + 가중 손실**로 다뤄 지도에 공정한 강한 셋업.
- **고정 균형 테스트셋**(정상40+결함40)에서 평가하고, **5 seed** 반복으로 분산까지 보고
  (단일-seed 가짜 양성 회피 — 지식증류 트랙과 같은 원칙).

**결과 (MVTec screw, 5-seed image AUROC, mean ± std):**

| 결함 라벨 수 | 패러다임 | image AUROC | 정확도 |
|---|---|---|---|
| **0** | **무지도 PatchCore** | **0.760 ± 0.055** | 0.74 |
| 2 | 지도 CNN | 0.587 ± 0.084 | 0.54 |
| 5 | 지도 CNN | 0.648 ± 0.043 | 0.57 |
| 10 | 지도 CNN | 0.747 ± 0.044 | 0.68 |
| **20** | **지도 CNN** | **0.829 ± 0.059** | 0.73 |
| 40 | 지도 CNN | 0.868 ± 0.015 | 0.74 |
| 79 | 지도 CNN | 0.925 ± 0.011 | 0.83 |

![지도 vs 무지도 라벨 희소성 곡선](data/results/head_to_head_screw.png)

**▶ 교차점 ≈ 결함 라벨 20개.** 라벨이 그보다 적으면 **무지도가 정답**이고, 많아지면 지도가 앞선다.
이게 모델 선택 플레이북의 "결함 라벨이 없거나 극소수" 행을 *말이 아니라 곡선으로* 뒷받침한다.

읽는 법(정직한 디테일까지):
- **소수 라벨일수록 지도는 불안정하다**: N=2에서 분산이 가장 크고(±0.084) AUROC가 우연(0.5) 근처까지
  떨어진다 — 2장에 과적합. 라벨이 늘수록 분산이 줄고(N=79 ±0.011) 성능이 단조 상승한다.
- **N=10은 무지도와 사실상 동률**(0.747 vs 0.760) — 교차는 점이 아니라 ~10–20 구간의 *밴드*다.
- 무지도 자체도 테스트 분할에 따라 흔들린다(±0.055) — 그래서 한 seed가 아니라 5 seed로 본다.
- screw는 결함이 작고 회전·이동해 image-level이 어려운 카테고리라 무지도 절대값이 0.76에 그친다
  (heavy 백본 wide_resnet50_2면 0.82까지 — 위 무지도 트랙 표). 여기선 *공정한 통제*를 위해 양쪽 다
  resnet18로 맞췄다. 절대 성능이 아니라 **두 패러다임의 상대 위치**가 이 트랙의 산출물이다.

```bash
python scripts/head_to_head.py --smoke                       # 순수 로직 자기점검(데이터 불요)
python scripts/head_to_head.py --seeds 5 --epochs 20         # 전체 실측(GPU, 약 50분)
```
...
| **결함 라벨이 없거나 극소수(정상만 풍부)** | **무지도 AD (PatchCore/PaDiM)** | 정상만 학습해 결함 위치화 (pixel AUROC 0.98). **같은 데이터 정면비교서 라벨 ~20개 미만이면 지도보다 우위**(교차점 실측) | **이상탐지 · head-to-head** |
...
+ 무지도 AD 히트맵(screw) 탭 + 플레이북. 데모는 각 패러다임을 제 데이터로 보이고, **같은 데이터 위의
정량 정면비교는 `scripts/head_to_head.py`**(위 "지도 vs 무지도" 트랙, 교차점 ≈ 라벨 20개)가 담당한다.
