...
- **v4는 재학습 레시피를 기준선급으로 복원**(rank32/α64 · 라벨 스무딩 0.1 · 증강 · **val 기준
  ...
  모델로 승격됐다.
  - ⚠️ *단서:* v1-bootstrap과 v4는 **LoRA 설정이 동일**(r32/α64)한데도 +13%p 차이가 났다. 차이는
    증강·val early-stopping 레시피와 교정 32건뿐이므로 이 점프의 상당 부분은 학습 레시피·체크포인트
    선택 차이로 보이나, 기준선을 같은 하니스로 재평가해 apples-to-apples 델타를 확정하진 못했다.
